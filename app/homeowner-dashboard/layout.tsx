...
const NAV_LINKS = [
  { href: '/homeowner-dashboard', label: 'My Report', icon: LayoutDashboard },
  { href: '/homeowner-dashboard/settings', label: 'Settings', icon: Settings },
...
export default function HomeownerDashboardLayout({
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      // Check for active homeowner subscription by user_id
      ...
      if (!subscription) {
        ...
        if (userEmail) {
          ...
          if (unlinked) {
            ...
            if (linkError) {
              console.error('Failed to link homeowner subscription:', linkError);
              router.replace('/homeowner');
              return;
            }
  ...
  const sidebarContent = (
    <>
      ...
      <div className="px-6 py-5 border-b border-slate-200">
        <Link href="/homeowner-dashboard" className="text-xl font-bold text-slate-900">
          List<span className="text-blue-600">AI</span>
        </Link>
        <p className="text-xs text-slate-500 mt-0.5">My Dashboard</p>
      </div>
